 ...
 * can attach a screenshot to someone else's booking. The customer never states an
 * amount — the admin reads it off the image — so nothing here is client-priced.
 */
export async function POST(request: Request) {
  try {
    ...
    const updated = await addPaymentAttempt({ bookingId: booking._id, screenshotKey: input.paymentScreenshotKey });
