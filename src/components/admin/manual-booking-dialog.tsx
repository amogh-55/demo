...
import { Check, ChevronDown, IndianRupee, MapPin, Phone, User, X } from "lucide-react";
...
export function ManualBookingDialog({
  ...
  const loadDay = React.useCallback(async () => {
    if (!open || !resourceId || !date) return;
    setLoadingDay(true);
    setDayError(null);
    try {
  ...
  const contiguous =
    ...
    selectedUnits.every((u, i) => i === 0 || u.startMin === selectedUnits[i - 1]!.endMin);
  ...
  const total = isOvers
    ? (ball?.pricePerSlot ?? 0) * slotsNeeded
  ...
  async function save() {
    ...
    try {
      ...
      onCreated(`Booking ${created.booking.reference} added for ${name.trim()}.`);
      setOpen(false);
      reset();
    } catch (err) {
  ...
  return (
    ...
    >
      ...
      <Dialog.Portal>
        <Dialog.Overlay className="fixed inset-0 z-40 bg-ink-950/60 backdrop-blur-sm" />
        {/* A sheet on a phone, a panel on a desktop; the footer never scrolls away. */}
        {/* Takes the height that is there rather than a fraction of it: on a short
            laptop window the old panel left a scroll box a few lines tall. Two
            columns from md up halve the scrolling again. */}
        <Dialog.Content className="theme-light fixed inset-x-0 bottom-0 z-50 flex h-[94dvh] flex-col overflow-hidden rounded-t-2xl bg-ink-50 shadow-2xl focus:outline-none sm:inset-x-auto sm:left-1/2 sm:top-1/2 sm:h-[min(52rem,calc(100dvh-1.5rem))] sm:w-[min(56rem,calc(100vw-1.5rem))] sm:-translate-x-1/2 sm:-translate-y-1/2 sm:rounded-2xl">
          <header className="flex items-start justify-between gap-3 border-b border-ink-200 bg-white px-5 py-4">
            <div className="flex min-w-0 items-center gap-3">
              <span className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-pitch-600 text-white">
                <Phone className="h-5 w-5" aria-hidden="true" />
              </span>
              <div className="min-w-0">
                <Dialog.Title className="text-base font-bold text-ink-900">Add a phone booking</Dialog.Title>
                <Dialog.Description className="text-sm text-ink-600">
                  Confirmed as soon as you add it, and the slot comes off the website.
                </Dialog.Description>
              </div>
            </div>
            <Dialog.Close
              className="-mr-1 grid h-9 w-9 shrink-0 place-items-center rounded-full text-ink-500 hover:bg-ink-100"
              aria-label="Close"
            >
              <X className="h-5 w-5" />
            </Dialog.Close>
          ...
          <div className="min-h-0 flex-1 overflow-y-auto p-3 sm:p-4">
            ...
            >
              {loadingDay ? (
                ...
                <Alert tone="warning">
                  This ground is closed on {formatBusinessDate(date)} — {day.dayBlock.reason}. Re-open it under
                  Availability first.
                </Alert>
              ) : splitByQuarter ? (
          ...
          <footer className="border-t border-ink-200 bg-white px-4 py-3 pb-[calc(0.75rem_+_env(safe-area-inset-bottom))]">
            ...
            <div className="mt-3 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
              <Dialog.Close asChild>
                <Button variant="secondary" className="h-11" disabled={saving}>
                  Cancel
                </Button>
              </Dialog.Close>
